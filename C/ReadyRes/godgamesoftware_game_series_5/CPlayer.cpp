...
void CPlayer::KeyUpdate(SHORT wParam) {
	int i;
	if(wParam==VK_LEFT) { //Pressed Left Key
		...
		if(AlphaSpeed<MAX_SPEED) AlphaSpeed++;
		PreKey = wParam;
	} else if(wParam == VK_SPACE) { //Pressed Space (Attack)
		Attack();
	}
}
...
void CPlayer::Attack() {
	Bullet->PushBody(X+Size/2,Y);
}

void CPlayer::Update(CEnemy *Enemy, CHp *Hp) {
	PopBody();
	Bullet->Update();
