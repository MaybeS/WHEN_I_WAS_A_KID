...
On Error Resume Next
                SaveSetting "Ayou", "App", "Install", False
    MkDir N_Locate & "\Ayou"
    UnzipModule.Unzip "c:\temp\Ayou.ins", N_Locate & "\Ayou"
    If InsOp(4) Then _
        FileCopy N_Locate & "\Ayou\Ayou.exe", "C:\Users\May\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\Ayou.exe"
    If InsOp(5) Then _
        MkDir "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Ayou": _
        FileCopy N_Locate & "\Ayou\Ayou.exe", "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Ayou\Ayou.exe": _
        FileCopy N_Locate & "\Ayou\읽어보세요.rtf", "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Ayou\읽어보세요.rtf"
    
    If InsOp(7) Then _
        MkDir N_Locate & "\Ayou\SendMessage": _
        SaveSetting "Ayou", "App", "SendMessage", True
    ...
    RmDir "c:\temp"
                ...
                SaveSetting "Ayou", "App", "Locate", N_Locate & "\Ayou"
                SaveSetting "Ayou", "App", "Install", True
    If InsOp(6) Then _
        Shell N_Locate & "\Ayou\Ayou.exe"
End Sub
...
Select Case State
    Case 0
        ...
        TextPrint fMain, 30, -10, "Ayou Installer", RGB(RedCntF, GreenCntF, BlueCntF), 64
    ...
    Else
        ...
        TextPrint fMain, 70, 170 + (30 * i), MSG(i), RGB(0, 0, 3), 12
            Next i
    End If
        TextPrint fMain, 10, 375, "AI 버전 : " & Ver, RGB(50, 60, 70), 10
    ...
    Case 1
        TextPrint fMain, 160, 5, "Ayou Install", RGB(RedCntF, GreenCntF, BlueCntF), 48
        
        TextPrint fMain, 20, 100, "설치할 항목을 선택하세요.", RGB(50, 70, 90), 20
            TextPrint fMain, 60, 170, "Ayou SE               (필수)", RGB(50, 70, 90), 20
            TextPrint fMain, 60, 200, "Ayou Plus             (추가 기능)", RGB(50, 70, 90), 20
            TextPrint fMain, 60, 230, "Ayou : Cris          (대화형 명령)", RGB(50, 70, 90), 20
            TextPrint fMain, 60, 260, "Ayou MoD            (대화체 설정)", RGB(50, 70, 90), 20
            TextPrint fMain, 60, 290, "Ayou MoD Maker   (개발자용)", RGB(50, 70, 90), 20
    ...
    Case 2
        ...
        TextPrint fMain, 20, 100, "설치될 위치를 확인해주십시오.", RGB(50, 50, 50), 20
            N_string = "\Ayou": If Len(N_Locate) > 33 Then N_string = "...\Ayou": _
            TextPrint fMain, 40, 180, N_Locate & "\Ayou", RGB(50, 50, 50), 8
        TextPrint fMain, 40, 150, Left(N_Locate, 33) & N_string, RGB(50, 50, 50), 15
        TextPrint fMain, 550, 150, "...", RGB(50, 50, 50), 16
        
        TextPrint fMain, 30, 200, "기본경로=C:\Users\" & UserName_ & "\AppData\Local\Ayou", RGB(50, 50, 50), 12
    ...
    Case 3
        ...
        TextPrint fMain, 50, 150, "윈도우 시작시 Ayou 시작", RGB(50, 50, 50), 16
        TextPrint fMain, 50, 180, "시작메뉴에 Ayou 폴더 만들기", RGB(50, 50, 50), 16
        TextPrint fMain, 50, 210, "설치를 마친후 Ayou를 실행합니다.", RGB(50, 50, 50), 16
        TextPrint fMain, 50, 240, "Ayou 사용정보를 개발자에게 보냅니다.", RGB(50, 50, 50), 16
        TextPrint fMain, 60, 275, "Ayou 외에 다른 자료는 보내지지 않습니다.", RGB(50, 50, 50), 12
    ...
    Case 4
        TextPrint fMain, 160, 5, "Ayou Install", RGB(RedCntF, GreenCntF, BlueCntF), 48
        TextPrint fMain, 370, 100, "MaybeStudio Present", RGB(150, 100, 133), 16
        TextPrint fMain, 30, 140, "Ayou의 설치가 완료되었습니다.", RGB(50, 50, 50), 20
        TextPrint fMain, 40, 190, "다음에 접속하여 Ayou에 대한 더많은 정보를 획득할 수 있습니다.", RGB(50, 50, 50), 12
        ...
        TextPrint fMain, 40, 330, "Ayou에 대한 저작권은 Maybe Studio 에 있습니다.", RGB(50, 50, 50), 16
...
On Error Resume Next
    ...
    FileCopy App.Path & "\Ayou.inf", "c:\windows\Uz.dll"
    MkDir "c:\temp"
        OpenInstall
...
On Error GoTo TCK
    FileCopy App.Path & "\AyouInstall", "c:\temp\AyouInstall"
    Kill "c:\temp\Ayouinstall"
    UnzipModule.Unzip App.Path & "\AyouInstall", "C:\Temp": Exit Sub
